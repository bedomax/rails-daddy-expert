...
5. Scan the diff for: `Model.find(params[:id])` without tenant scope → ❌ IDOR; `Model.all` without scope → ❌; `permit(:tenant_id/:role_id)` → ❌
6. For each new migration: `_id` columns have `add_index` ✅/❌; `null:` declared ✅/❌
7. `git diff $BASE..HEAD --name-only | grep 'app/controllers' | sed 's|app/|spec/|;s|\.rb|_spec.rb|' | xargs grep -l "other_tenant" 2>/dev/null` — isolation test present ✅/❌
